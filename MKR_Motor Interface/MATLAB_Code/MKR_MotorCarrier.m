classdef MKR_MotorCarrier < handle
    ...
    %
    % all incoming data is processed by the read() function, which calls
    % itself in the background whenever there is data available over the
    % wireless UDP Connection. Data is sorted into appropriate buffers
    % accodring to its type, which is determined on the Arduino end.
    % Buffers for each different type are availble for the peripherals used
    % in the labs and projects connected with this course.
    %
    %
    ...
    properties
        u; robotIP; robotPORT; ready; sizeofBuffer; tempSignal; tempData;...
            ...
            RDB_Toggle; RDB_Index; channelToDraw; time; red; green; blue; irBuffer;...
        encoderPos; encoderVel; ultrasonicBuffer; irSensorBuffer; newIrIndex; ultrasonicSensorBuffer; newUltraIndex
    ...
    methods
        ...
        function obj = MKR_MotorCarrier(varargin)
            ...
            obj.plotDelay = 1;
            obj.plotDelayMax = 50;
            obj.available_streams = ["analog", "imu", "ir", "ultra"];
            obj.streamToDraw = "";
            obj.channelToDraw = 0;
            ...
            obj.RecordedDataBuffer = zeros(12,obj.sizeOfRecordedDataBuffer);
            ...
            obj.ultrasonicSensorBuffer = zeros(1,obj.sizeofBuffer);
            obj.irSensorBuffer = zeros(4,obj.sizeofBuffer);
        ...
        function pinMode(obj, pin, io)
            ...
            if ~ismember(io, ["INPUT", "OUTPUT", "INPUT_PULLUP"])
        ...
        function vals = readReflectance(obj)
            % READREFLECTANCE returns the last four values from the
            % infrared sensor
        ...
        function values = getAverageData(obj, streamType, varargin)
            ...
            switch(streamTypeCorrected)
                ...
                case "ir"
                    if nargin == 3 && varargin{1} > 0
                        values = mean(obj.irSensorBuffer(:,end-varargin{1}:end), 2);
                    else
                        values = mean(obj.irSensorBuffer, 2);
                    end
                case "ultra"
                    if nargin == 3 && varargin{1} > 0
                        values = mean(obj.ultrasonicSensorBuffer(:,end-varargin{1}:end), 2);
                    else
                        values = mean(obj.ultrasonicSensorBuffer,2);
                    end
                otherwise
            end
        end
        ...
        function values = getNewData(obj, streamType)
            ...
            switch(streamTypeCorrected)
                ...
                case "imu"
                    ...
                    obj.newImuIndex = obj.sizeofBuffer;
                case "ir"
                    values = obj.irSensorBuffer(:,obj.newIrIndex + 1:end);
                    obj.newIrIndex = obj.sizeofBuffer;
                case "ultra"
                    values = obj.ultrasonicSensorBuffer(:,obj.newUltraIndex + 1:end);
                    obj.newUltraIndex = obj.sizeofBuffer;
            end
        end
        ...
        function resetEncoder(obj)
            sendOverUDP(obj,15,1,1);
        end

        function [val1,val2] = readEncoderPose(obj)
            % READENCODERPOSE returns two encoder values to determine the
            % encoder position.
        ...
        function [val1,val2] = readEncoderVel(obj)
            % READENCODERVEL returns two encoder values to determine the
            % encoder velocity
        ...
        function f = checkFrequency(obj, t)
            obj.startRecording;
            pause(t);
            f = obj.stopRecording;
            f = length(f)/t;
    ...
    methods(Hidden=true)
        ...
        function obj = init(obj, varargin)
            ...
            % only called internally when the constructor is ran.
            ...
            for i = 1:datagramCount
                ...
                switch(obj.tempSignal(1))
                    case "STR"
                        ...
                        if sum(obj.tempData(8:11) ~= 0)
                            obj.irSensorBuffer = circshift(obj.irSensorBuffer, -1, 2);
                            obj.irSensorBuffer(:,end) = obj.tempData(8:11);
                            if obj.newIrIndex > 1
                                obj.newIrIndex = obj.newIrIndex - 1;
                            end
                        end
                        if obj.tempData(12) ~= 0
                            obj.ultrasonicSensorBuffer = circshift(obj.ultrasonicSensorBuffer, -1, 2);
                            obj.ultrasonicSensorBuffer(:,end) = obj.tempData(12);
                            if obj.newUltraIndex > 1
                                obj.newUltraIndex = obj.newUltraIndex - 1;
                            end
                        end
